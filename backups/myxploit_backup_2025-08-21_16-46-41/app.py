...
def transport():
    ...
    if request.method == "POST":
        ...
        niveau_calcul = request.form.get("niveau_calcul", "")
        ...
        charge_utile = float(request.form.get("charge_utile", "0") or 0)
        type_vehicule = request.form.get("type_vehicule", "")
        energie = request.form.get("energie", "")
        
        # Récupération des dates de planning (seulement pour les transports indirects)
        date_commande = request.form.get("date_commande", "")
        ...
        # Structure de base du transport
        transport_data = {
            "ref": ref,
            "type_transport": type_transport,
            "niveau_calcul": niveau_calcul,
            "date": date,
            ...
            "energie": energie,
            "conso_vehicule": float(request.form.get("conso_vehicule", "0") or 0),
            "emis_kg": emis_kg,
            "emis_tkm": emis_tkm,
            "phases": []
        }
        
        # Ajouter les dates de planning seulement pour les transports indirects
        if type_transport == "indirect":
...
@app.route("/transport/<ref>/edit", methods=["GET", "POST"])
@login_required
def edit_transport(ref):
    """Route pour éditer un transport existant"""
    clients, energies, transports = normalize_data_simple()
    vehicules = load_json("vehicules.json")
    
    # Récupérer le transport à éditer
    transport = transports.get(ref)
    if not transport:
        flash(f"Transport {ref} non trouvé", "error")
        return redirect(url_for("liste_transports"))
    ...
    if request.method == "POST":
        ...
        # Calcul des émissions basé sur l'énergie, la distance et la consommation
        emis_kg = 0
        emis_tkm = 0
        ...
        if energie and distance_km > 0 and conso_vehicule > 0:
            # Récupérer le facteur d'émission de l'énergie
            energie_data = energies.get(energie)
            if energie_data and energie_data.get("facteur"):
                facteur = float(energie_data["facteur"])
                # Formule: (distance/100 × consommation) × facteur
                emis_kg = (distance_km / 100 * conso_vehicule) * facteur
                emis_tkm = facteur
        
        # Mise à jour du transport
        transports[ref].update({
            "type_transport": type_transport,
            "niveau_calcul": niveau_calcul,
            "date": date,
            "ville_depart": ville_depart,
        ...
        if type_transport == "indirect":
            ...
            # Supprimer les dates de planning pour les transports directs
            transports[ref].pop("date_commande", None)
            transports[ref].pop("date_collecte", None)
            transports[ref].pop("date_livraison", None)
        
        save_json("transports_simple.json", transports)
        flash(f"Transport {ref} modifié avec succès !", "success")
        return redirect(url_for("liste_transports"))
...
def transport_detail(ref):
    ...
    # Déterminer le facteur d'émission (utiliser directement l'énergie du transport)
    vehicule_facteur = 0
    energie_transport = t.get("energie", "")
    if energie_transport and energie_transport in energies:
        vehicule_facteur = energies[energie_transport].get("facteur", 0)
